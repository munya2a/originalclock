...
static void	appwindow_activate(GtkApplication *pApp, gpointer pData) {
	...
	// ttps://stackoverflow.com/questions/43029025/c-gtk-g-application-quit?rq=1
...
static gboolean appwindow_draw(GtkWidget *pWidget, cairo_t *cr, gpointer pData) {
	...
	if (mode & MODE_ANALOG) {
		...
		rr = (fontColor & 0xff0000) >> 16;
		rr /= 255;
		gg = (fontColor & 0x00ff00) >>  8;
		gg /= 255;
		bb = fontColor & 0x0000ff;
		bb /= 255;
...
void* timer_thread(void *threadid) {
	...
	for (;;) {
		...
		// When displaying minutes and seconds
		} else {
			gtk_widget_queue_draw(g_pAppWnd);
			::sleep(1);
		}
...
//
// url: http://zetcode.com/gfx/cairo/root/
//
//--------------------------------------------------------------------------------------------------
void	init_bktransprnt(GtkWidget *pWidget) {
...
long	SimpleFileRead(const char *p_path, byte *p_buffer, long size) {
	...
	if (fileSizeLow > 0x7fffffff || fileSizeLow < 0) {
		return 0;
	}
